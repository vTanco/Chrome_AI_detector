...
class AITextDetector {
  static analyze(text, sensitivity = "balanced") {
    ...
    // Los textos menores a 10 palabras carecen de suficiente señal estadística
    if (wordCount < 10) {
      return {
        score: 0,
        level: "none",
        eligible: false,
        wordCount,
        reasons: ["Fragmento demasiado corto para análisis fiable (mínimo 10 palabras)."]
      };
    }
    ...
    // Filtrar encabezados, títulos y etiquetas sin puntuación terminal para no falsear la varianza
    ...
    // Coeficiente de variación (CV): Humano > 0.45, IA suele rondar 0.10 - 0.35
    ...
    // Factor B: Densidad de clichés y giros discursivos de IA
    const clicheDensity = (foundCliches.length / wordCount) * 100;
    if (foundCliches.length >= 3 || clicheDensity >= 1.0) {
      score += 35;
    ...
    if (structuralPoints > 0) {
      indicators.push("Estructura formulaica, viñetas estandarizadas o preguntas retóricas de IA detectadas.");
    }

    // Factor E: Peritaje específico de laboratorio (OpenAI, Anthropic Claude, Google Gemini)
  ...
  static detectLanguage(text, words) {
    ...
    const spanishCommonWords = new Set([
      ...
      "o", "no", "si", "su", "sus", "este", "esta", "estos", "estas", "como",
      "más", "pero", "sobre", "entre", "sin", "hasta", "desde", "también",
      "está", "están", "muy", "todo", "todos", "toda", "todas", "ya", "ha", "han"
    ]);
    ...
    return (spanishMatches >= 3 || (spanishMatches / words.length) >= 0.15) ? "es" : "en";
  }
